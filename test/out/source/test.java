    ...
    if(unhex(co[0]) == 65535){      //co[0]がFFFFなら
        if (time >= 200) {          //0.2秒ずつ
            ...
            if(head == 0){      //始まりの座標
                ...
                end[i] = new PVector(PApplet.parseInt(co[1]),PApplet.parseInt(co[2]),PApplet.parseInt(co[3]));
                start[i + 1] = end[i];
                i++;
            }
            ln++;       //1行増やす
        ...
        if(time >= 200){
            end[i] = new PVector(PApplet.parseInt(co[1]),PApplet.parseInt(co[2]),PApplet.parseInt(co[3]));     //最後の座標
        i++;
        ln++;
        head = 0;
        base_time = millis();
